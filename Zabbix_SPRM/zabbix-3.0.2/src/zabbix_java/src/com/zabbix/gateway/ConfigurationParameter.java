...
class ConfigurationParameter
{
	...
	public static final int TYPE_INETADDRESS_LIST = 3;
		...
		try
		{
			switch (type)
			{
				...
				case TYPE_FILE:
					...
					break;
				// Add H.Tan 2016/06/16
				case TYPE_INETADDRESS_LIST:
					userValue = getInetAddressList(text);
					break;
			}
		}
		catch (Exception e)
	...
	// Add H.Tan 2016/06/16
	private List<InetAddress> getInetAddressList(String text) {
		List<InetAddress> ipList = new ArrayList<InetAddress>();
		if (text != null) {
		    String[] list = text.split(",");
		    for (int i = 0; i < list.length; i++) {
		    	try {
		    		ipList.add(InetAddress.getByName(list[i].trim()));
