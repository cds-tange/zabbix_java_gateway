...
class JMXItemChecker extends ItemChecker
{
	...
	public JMXItemChecker(JSONObject request) throws ZabbixException
	{
		...
		try
		{
			...
			// Add H.Tan 2016/06/16
			url = null;
			InetAddress jmxIp = InetAddress.getByName(conn);
			String connIp = jmxIp.getHostAddress();
			if (jmxIp instanceof Inet6Address) connIp = "[" + connIp + "]";
			@SuppressWarnings("unchecked")
			List<InetAddress> wildFlys = (List<InetAddress>)ConfigurationManager.getParameter(ConfigurationManager.WILD_FLYS).getValue();
			Iterator<InetAddress> iter = wildFlys.iterator();
			while (iter.hasNext()) {
				if (iter.next().getHostAddress().equalsIgnoreCase(jmxIp.getHostAddress())) {
					url = new JMXServiceURL("service:jmx:remote+http://" + connIp + ":" + port);
					break;
				}
			}

			if (url == null) {
				url = new JMXServiceURL("service:jmx:rmi:///jndi/rmi://" + connIp + ":" + port + "/jmxrmi");
			}
